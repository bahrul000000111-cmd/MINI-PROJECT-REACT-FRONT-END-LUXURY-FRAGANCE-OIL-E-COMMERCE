...
const socials = [
  {
    ...
    icon: (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>),
  },
  {
  ...
  return (
    <footer style={{ background: "#1A1A1A", color: "white", padding: "60px 24px 32px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {socials.map((s) => (
                <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer"
                  style={{ width: 34, height: 34, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.55)", textDecoration: "none", transition: "border-color 0.2s, color 0.2s" }}
        ...
        {/* Newsletter */}
        <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 16, padding: "24px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, marginBottom: 40, border: "1px solid rgba(255,255,255,0.08)", flexWrap: "wrap" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input type="email" placeholder="Your email address"
              style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 999, padding: "10px 18px", color: "white", fontSize: 12, outline: "none", width: 220 }}
            />
            <button
              style={{ background: "white", color: "#1A1A1A", borderRadius: 999, padding: "10px 20px", fontSize: 12, fontWeight: 600, border: "none", cursor: "pointer", transition: "background 0.2s" }}
        ...
        {/* Bottom bar */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 24, borderTop: "1px solid rgba(255,255,255,0.08)", flexWrap: "wrap", gap: 12 }}>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>
            © 2026 Frägra. All rights reserved.
          </p>
          <div style={{ display: "flex", gap: 20 }}>
            {bottomLinks.map(({ label, slug }) => (
              <Link key={label} to={`/info/${slug}`}
                style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", textDecoration: "none", transition: "color 0.2s" }}
